 ...
 * <pre>
 * <?php $form = $this->beginWidget('webnula2\widgets\booster\TbActiveForm', array(
 *     'type' => 'horizontal',
...
class TbActiveForm extends \CActiveForm
{
	...
	public function datePickerGroup($model, $attribute, $options = array()) {
		...
		return $this->widgetGroupInternal('webnula2\widgets\booster\TbDatePicker', $model, $attribute, $options);
	}

	/**
	 * Generates a tinymce group for a model attribute.
	 *
	 * @param $model
	 * @param $attribute
	 * @param array $options
	...
	public function tinymceGroup($model, $attribute, $options = array()) {
		$options['widgetOptions'] = array_merge(array(
			...
			'fileManager' => array(
				...
				'popupTitle' => \Yii::t('webnula2.locale', "Files"),
			),
		), $options['widgetOptions']);
		return $this->widgetGroupInternal('webnula2.extensions.tinymce.TinyMce', $model, $attribute, $options);
	}
