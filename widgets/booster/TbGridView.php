...
class TbGridView extends \CGridView
{
	 ...
	 */
	public $pager = array('class' => 'webnula2\widgets\booster\TbPager');
	...
	protected function initColumns()
	{
		foreach ($this->columns as $i => $column) {
			if (is_array($column) && !isset($column['class'])) {
				$this->columns[$i]['class'] = 'webnula2\widgets\booster\TbDataColumn';
			}
		}
	...
	protected function writeResponsiveCss()
	{
		...
		foreach ($this->columns as $column) {
			...
			$name = htmlspecialchars_decode( strip_tags(ob_get_clean()) );
