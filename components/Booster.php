...
class Booster extends \CApplicationComponent
{
	...
	protected function setRootAliasIfUndefined()
	{
		if ( \Yii::getPathOfAlias( 'booster' ) === false ) {
			\Yii::setPathOfAlias( 'booster', \Yii::getPathOfAlias( 'webnula2.assets' ) );
		}
	}
	...
	protected function appendUserSuppliedPackagesToOurs()
	{
		...
		$bootstrapPackages = require( \Yii::getPathOfAlias( 'webnula2.components' ) . '/packages.php' );
	...
	public function getAssetsUrl()
	{
		...
		if ( isset( $this->_assetsUrl ) ) {
			...
			return $this->_assetsUrl = (YII_DEBUG ?
				\Yii::app()->getAssetManager()->publish(\Yii::getPathOfAlias('booster'), false, -1, true) :
				\Yii::app()->getAssetManager()->publish(\Yii::getPathOfAlias('booster'))
			);
		}
	}
	...
	protected function createSelect2Package()
	{
		...
		if ( strpos( \Yii::app()->language, 'en' ) !== 0 ) {
			...
			if ( @file_exists( \Yii::getPathOfAlias( 'booster.select2' ) . DIRECTORY_SEPARATOR . $locale ) ) {
				$jsFiles[] = $locale;
			} else {
